...
- **Two flavors:** **TPP** (Trust Protection Platform, on-prem — typical enterprise) uses `url`+`zone`+token; **Venafi Cloud** (VaaS) uses `zone`+API key.
- Config (TPP): `spec.venafi.zone: "Org\\Platform\\OpenShift"` (policy folder) + `tpp.url` + `credentialsRef` (Secret w/ access token). Certificate references this ClusterIssuer → cert-manager calls Venafi → policy applied → corp-CA-signed cert into Secret.
...
### Annotated Certificate CR (this is a CR, not the CRD)
```yaml
...
spec:
  ...
  secretTemplate:
    annotations: {kyverno.example.com/reload: "allow"}  # Kyverno restarts pods on rotation (Reloader-style)
  ...
  commonName / dnsNames: patchme     # placeholders → real hostname patched by templating CLI at deploy
  issuerRef: {name: standard-venafi-issuer, kind: VenafiClusterIssuer, group: jetstack.io}  # Venafi Enhanced Issuer
```
- `{{.cleanedAppName}}` etc = **Go-template rendered by an internal templating CLI** (a `renderedBy` label records the version); file in Git is a template, applied object is rendered.
...
`Git (Subscriptions, VenafiClusterIssuer, Kyverno policies…) → Argo CD Application syncs → OLM installs operators → CRDs appear`. **Argo sits ABOVE OLM.** the templating CLI renders templates → Git → Argo syncs.
...
### The 3 Argo CD objects
| object | what | think of it as |
|--------|------|----------------|
| **Application** | maps Git source (repo/path/rev) → destination (cluster+ns) + sync policy | "sync this repo folder into this ns" |
| **AppProject** | grouping + **security boundary**: restricts allowed repos/destinations/resource kinds + RBAC | the **multi-tenancy guardrail/fence** |
| **ApplicationSet** | **generator** templating many Applications (list/cluster/git-dir/git-file/matrix generators) | factory stamping 1 App per cluster/team/folder |
- Relate: AppProject = fence (every App belongs to one, `default` if unset) ⊃ Application = deploy unit; ApplicationSet = factory. Multi-tenant = AppProject isolates teams + ApplicationSet scales across clusters.
...
### The 3 analysis objects (template→run pattern)
| object | what | think of it as |
|--------|------|----------------|
| **AnalysisTemplate** | reusable **definition**: metric queries + success/fail conditions | the blueprint |
| **AnalysisRun** | one **execution** of the template, fired at a canary step | Template:Run :: Pipeline:PipelineRun |
| **Experiment** | short-lived ReplicaSet(s) + own AnalysisRuns, bounded time, off to the side | A/B test / pre-flight (not in normal canary path) |
- Flow: canary `analysis` step → spawns AnalysisRun (from AnalysisTemplate) → queries Prometheus → pass=continue / fail=auto-rollback. Experiment = throwaway parallel test (A/B, validate build before real canary).
...
### 🔁 Alternatives
GitHub Actions (JD) · GitLab CI · Jenkins · **Tekton = OpenShift Pipelines**. Registry: ECR/Quay/Artifact Registry. Cloud auth = OIDC (no keys).
...
## Block K — Supply-Chain Security (Tekton Chains + RHTAS/Sigstore) [OUR STACK]

### Problem
Trust an image? Supply-chain attacks inject malware between code→cluster. Fix = **sign artifacts + record proof**. Framework = **SLSA**; toolkit = **Sigstore**.

### Sigstore stack = the artifact-signer overlay (RHTAS, on a **central signing cluster**)
| component | role |
|-----------|------|
| **Fulcio** | keyless **CA** — short-lived signing cert bound to OIDC identity |
| **Rekor** | immutable **transparency log** (every signature) |
| **CT Log** | Cert Transparency log for Fulcio certs |
| **Trillian** | **Merkle-tree** backend → Cloud SQL (Postgres) via Cloud SQL Proxy |
| **TUF server** | distributes **trust roots** |
| **Rekor Search UI** | query the log |
- ⭐ **Keyless signing** = same OIDC federation theme: no long-lived signing key; Fulcio issues short-lived cert for an OIDC identity.

### Tekton Chains (controller in `openshift-pipelines`, SA `tekton-chains-controller`)
- Observes TaskRuns/PipelineRuns → on build completion: **signs image** (Fulcio cert) + generates **SLSA provenance** (source commit, pipeline) + **records to Rekor**.
- Signing **identity:** `https://kubernetes.io/namespaces/openshift-pipelines/serviceaccounts/tekton-chains-controller`. Verify via `cosign verify --certificate-identity=...`.
- A **central signing cluster** hosts the signing infra; other clusters' Chains sign against it (one audit truth).

> Supply-chain soundbite: "Tekton Chains auto-signs every image a pipeline builds and records SLSA provenance to Rekor — keylessly via a short-lived Fulcio cert bound to the pipeline's OIDC identity, no key to leak. Backend is self-hosted RHTAS on a central signing cluster: Fulcio CA, Rekor log, Trillian Merkle store on Cloud SQL, TUF trust roots. Verification checks the image was signed by our pipeline identity and is in Rekor."

---

## Block L — Secrets Management (ESO + AVP) [OUR STACK]

### GitOps secrets problem
Can't put raw secrets in Git → keep in external vault (**GCP Secret Manager**), inject into cluster. Two mechanisms:

### A) External Secrets Operator (ESO) — runtime, continuous
- `ExternalSecret` CRD → controller fetches from source → **materializes a native K8s Secret** → keeps **refreshed** (`refreshInterval`). Reconciliation loop.
- ⭐ A **`QuayAccessToken` generator** = mints **short-lived robot pull-token** for the private registry every **55m** (tokens expire 1h), auth via **WIF** (`serviceAccountRef`) → always-fresh `dockerconfigjson` Secret. No static registry creds.

### B) ArgoCD Vault Plugin (AVP) — render/sync time, one-shot
- `AVP_TYPE=gcpsecretmanager`; `argocd-vault-plugin generate` replaces **placeholders** with GCP Secret Manager values at Argo render time. Good for static secrets.

### ESO vs AVP
| | **ESO** | **AVP** |
|---|---|---|
| when | runtime, continuous refresh | render/sync time, one-shot |
| what | controller + CRD | Argo plugin (placeholder sub) |
| best for | rotating/short-lived (tokens) | static secrets |
| our use | Quay pull tokens (55m) | GCP SM values in overlays |
- 🔁 **AWS:** ESO `SecretStore` → **AWS Secrets Manager / SSM Parameter Store**, auth via **IRSA**. AWS-native alt = **Secrets Store CSI driver**.

> Secrets soundbite: "Secrets never go in Git. ESO reconciles an ExternalSecret CRD into a native Secret from GCP Secret Manager and keeps it refreshed — we use a QuayAccessToken generator minting short-lived registry pull tokens every 55m via workload identity, so no static registry creds exist. ArgoCD Vault Plugin injects static GCP Secret Manager values at render time. On EKS the same ESO points at AWS Secrets Manager via IRSA."

---

## Block M — Platform Tooling We Own (quick ref)
| tool | what | where |
|------|------|-------|
| **Tekton Notify** | operator: event-driven **notifications from pipeline runs** (CRDs, WIF auth) | all clusters |
| **Tekton Observer** | operator: **watches pipeline lifecycle** for observability | clusters |
| **fuzzyhub** | custom **Tekton resolver** — resolves task/pipeline refs (fuzzy match) | pipelines overlay |
| **ArtifactHub** | web app to find/install/publish K8s packages (Helm/OPA/Tekton) — Cloud SQL backed | clusters |
| **Terraform Registry** (gcs-proxy) | proxy over **GCS bucket** serving private **TF modules/providers** | central cluster |
| **Bulldozer** (Palantir) | GitHub App: **auto-merge PRs** when checks pass | clusters |
| **Policy-bot** (Palantir) | GitHub App: **enforce PR approval policies** (policy file) | clusters |
- All = **Argo CD Applications** + templating-CLI overlays. Dev flow: PR → policy-bot (approvals) → bulldozer (auto-merge) → Tekton build (fuzzyhub resolves tasks) → Chains signs → Rekor (central signing cluster) → tekton-notify/observer report → Argo deploys + ESO pull tokens → Terraform (gcs-proxy registry) provisions infra.

### Component details learned (component-by-component tour)
- **Tekton:** Task=Pod, Step=container; Pipeline/PipelineRun = blueprint/run. `TektonConfig` CR (profile all) fans out → Pipelines/Triggers/Chains/Addon. EventListener: webhook → interceptors (GitHub HMAC + CEL filter) → TriggerBinding → TriggerTemplate → PipelineRun. Resolvers fetch defs remotely.
- **Chains keyless (the hard part):** ephemeral keypair → **Fulcio** issues ~10min cert for OIDC identity `tekton-chains-controller` → sign digest → **throw private key** (cert kept, holds public key). **Rekor** logs {digest,sig,cert,TIMESTAMP}. Verify: cosign checks cert validity window CONTAINS the **Rekor timestamp** (NOT "now") — that's why keyless needs Rekor (cert already expired at verify). **TUF** = distributes trust roots (Fulcio/Rekor/CTlog pubkeys), anchored in **offline threshold-signed root.json** (trust once out-of-band, chains+rotates via signatures; root keys offline/rare, timestamp key online/frequent/low-blast). RHTAS on a **central signing cluster**.
- **fuzzyhub:** custom Tekton **resolver**, fetches Task/Pipeline from a **GCS** modules bucket by **semver range** ("fuzzy"). Only fuzzyhub is in the runtime path (NOT ArtifactHub).
- **ArtifactHub:** human **catalog/discovery UI** (CNCF) — NOT a resolver, NOT related to Notify.
- **tf-registry:** SOURCE (templating CLI scaffolds modules → GCS; Stack Builder no-code) + SERVER (`gcsproxy` Go binary speaks TF registry protocol: `/.well-known/terraform.json` discovery + `/v1/modules`, auth GCS via **WIF `credentials_config.json`**=external_account; Cloud Run `tfregistry-...a.run.app`). Module addr = `HOST/NAMESPACE/NAME/SYSTEM` (`<org>/cloud-storage/gcp`).
- **Notify vs Observer:** feedback=all clusters (PR comments/email) vs aggregation=subset (counts→BigQuery→Grafana). Prove-deployed = overlay exists under `overlays/<cluster>`.
- **Bulldozer** (auto-merge when green) + **Policy-bot** (`.policy.yml` approval gate) = Palantir GitHub Apps; webhooks → need Ingress/TLS.
- **Scheduling recap:** taint(node repels)+toleration(pod permits, only allows not forces) — NoSchedule/PreferNoSchedule/NoExecute(evicts); nodeAffinity(pod→nodes, required/preferred); pod(anti)affinity(pod→pods, topologyKey); topologySpreadConstraints(maxSkew + whenUnsatisfiable DoNotSchedule/ScheduleAnyway).

---

## Block N — IDP: Red Hat Developer Hub (Backstage) [OUR STACK]
- **IDP = Internal Developer Platform/Portal** (NOT identity provider). RHDH = Red Hat's build of **Backstage** (CNCF, ex-Spotify; janus-idp operator). = self-service **front door** over all platform tooling. Deployed from a developer-hub GitOps repo into a dedicated namespace, CloudSQL-backed.
- **4 pillars:**
  | pillar | role |
  |--------|------|
  | **Software Catalog** | registry of services/APIs/resources + **owners** (from `catalog-info.yaml` per repo) |
  | **Scaffolder** (Software Templates) | ⭐ self-service **golden paths** — "Create Service" form → steps: scaffold repo + pipeline + catalog entry + infra |
  | **TechDocs** | docs-as-code (Markdown in repo → rendered in portal) |
  | **Plugins** | embed Argo CD / Tekton / K8s / dashboards = one pane of glass |
- **orchestrator** = OpenShift Serverless Logic (**SonataFlow**) = event-driven **workflow engine** sequencing multi-step self-service actions (step B after A).
- **github-receiver** = GitHub webhooks → keep catalog in sync. **techdocs** = TechDocs component. **terraform** = hub infra (CloudSQL/IAM).
- Same platform shape: OLM subscription (+ `installplan-approver-job` auto-approves), Argo CD delivery, AVP/GCP SM secrets, HPA/PDB/Ingress/cert-manager, keyless Quay pull.

> IDP soundbite: "An Internal Developer Platform like Backstage/RHDH is the self-service front door over all platform tooling. The Software Catalog tracks every service and owner from catalog-info.yaml; the Scaffolder gives golden-path templates so a dev clicks 'create service' and gets a repo, pipeline, catalog entry and infra auto-provisioned with signing/GitOps/quotas baked in. TechDocs renders docs-as-code and plugins surface Argo/Tekton status in one UI. We run RHDH via OLM and Argo CD, CloudSQL-backed, with a SonataFlow orchestrator driving the self-service workflows."
